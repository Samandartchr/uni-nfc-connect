  ...
  return (
    <AppLayout>
      ...
      <div className="relative mb-4">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search…" className="h-12 rounded-2xl border-0 bg-surface-low pl-11" />
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((f, i) => (
            <motion.button key={f.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.04 }}
