...
export default function FeedPage() {
  ...
  const toggleLike = (id: string) => {
    setLiked((prev) => {
      ...
      setPosts((ps) =>
        ps.map((p) =>
          p.id === id ? { ...p, likes_count: p.likes_count + (isLiked ? -1 : 1) } : p,
        ),
      );
      return next;
    });
  ...
  return (
    <AppLayout>
      ...
      {posts.length === 0 ? (
        <div className="rounded-3xl bg-surface-low p-10 text-center text-muted-foreground">
          {t("feed.empty")}
        </div>
      ) : (
            ...
            >
              <header className="mb-3 flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {p.faculty} · {timeAgo(p.created_at)}
                  </div>
                </div>
              </header>
              ...
              <div className="mt-3 flex items-center gap-4 text-xs text-muted-foreground">
                <button
                  ...
                  className={`inline-flex items-center gap-1.5 transition ${liked.has(p.id) ? "text-secondary" : "hover:text-foreground"}`}
                >
