      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-col items-center pt-2 text-center">
          <div className="relative">
            ...
            {p.is_pro && (
              <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full bg-gradient-primary px-3 py-0.5 text-[10px] font-bold tracking-wider text-primary-foreground shadow-glow">PRO</span>
            )}
          </div>
          {editing ? (
            <Input value={p.full_name} onChange={(e) => setDraft({ ...p, full_name: e.target.value })} className="mt-5 text-center text-2xl font-bold" />
          ) : (
            <h1 className="text-display mt-5 text-3xl">{p.full_name || "—"}</h1>
          )}
          {editing ? (
            <div className="mt-2 flex w-full max-w-xs gap-2">
              <Input value={p.faculty} placeholder={t("prof.faculty")} onChange={(e) => setDraft({ ...p, faculty: e.target.value })} />
        ...
        <div className="mt-5 flex justify-center gap-2">
          {editing ? (
            <>
              <Button onClick={save} variant="hero" size="sm"><Save className="h-3.5 w-3.5" />{t("prof.save")}</Button>
              <Button onClick={() => { setDraft(profile); setEditing(false); }} variant="ghost" size="sm"><X className="h-3.5 w-3.5" />{t("prof.cancel")}</Button>
            </>
          ) : (
            <Button onClick={() => setEditing(true)} variant="glass" size="sm"><Edit3 className="h-3.5 w-3.5" />{t("prof.edit")}</Button>
          )}
        </div>
