    ...
    <div className="relative mx-auto w-full max-w-md">
              ...
              {phase === "open" && (
                ...
                >
                  <div className="mx-auto h-16 w-16 rounded-full bg-gradient-primary p-0.5 shadow-glow-strong">
                    <div className="grid h-full w-full place-items-center rounded-full bg-surface-low text-2xl">👩‍🎓</div>
                  </div>
                  <div>
                    <div className="text-sm font-bold">Anna Kovaleva</div>
                    <div className="text-[10px] text-muted-foreground">Creative Director · StartUpLab</div>
                  </div>
                  <div className="space-y-1.5 text-left">
                    <div className="rounded-lg bg-surface-high px-2 py-1.5 text-[10px]">🔗 portfolio.link</div>
                    <div className="rounded-lg bg-surface-high px-2 py-1.5 text-[10px]">📷 @kovaleva</div>
                  </div>
                  <button className="w-full rounded-xl bg-gradient-primary py-2 text-xs font-bold text-primary-foreground shadow-glow">
                    <Plus className="mr-1 inline h-3 w-3" />{t("land.add")}
                  </button>
                  <button onClick={() => setPhase("idle")} className="w-full rounded-xl bg-surface-high py-1.5 text-[10px] text-muted-foreground">
                    <X className="mr-1 inline h-3 w-3" />{t("land.decline")}
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      >
        ...
        <span className="relative grid h-14 w-14 place-items-center rounded-full bg-gradient-primary shadow-glow-strong transition group-hover:scale-105">
          {phase === "open" ? <Check className="h-6 w-6 text-primary-foreground" /> : <Radio className="h-6 w-6 text-primary-foreground" />}
        </span>
      </button>
    </div>
  );
